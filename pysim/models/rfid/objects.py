...
class Listeners:
    ...
    def add(self, fn, **kwargs):
        index = next(self._id)
        self._items.append((index, fn, kwargs))  # Структура элементов items
        return index
...
class _ReaderRound:
    ...
    def next_slot(self):  # Этот метод чаще всех меняет состояние считывателя
...
class Reader:
    ...
    def receive(self, tag_frame):
        ...
        if isinstance(reply, std.QueryReply):
            ...
            raise ValueError(f'unexpected tag reply {str(reply)}')
...
class Tag:
    ...
    def persistence(self, session):  # TODO зачем такой механизм?
...
class Generator:
    """
    Класс, генерирующий новые метки?
    Handler`ы вызывают сначала этот класс, а он уже в свою очередь вызывает класс Tag
    """
...
class _TagPowerRecord:
    ...
    @tag_rx_power.setter
    def tag_rx_power(self, value):
        self._tag_rx_power = value
    ...
    def reader_tx_power(self, value):
