...
Модуль содержит определения основных физических объектов модели: антенны, ридер, метка, среда распространения сигнала. Используется при сборке модели в `configurator.py`. Здесь реализована логика расчёта мощности сигнала, потерь, направленности, включая учёт отражений и эффекта Доплера. Код не предназначен для запуска напрямую, но активно используется как внутренняя основа симуляции.
✅ Публичные классы (без подчёркивания):

Это основные интерфейсы модели, с которыми взаимодействует внешний код:
	•	Model
	•	Reader
	•	Tag
	•	Generator
...
Они представляют ключевые компоненты RFID-системы и предназначены для:
	•	конфигурирования,
	•	инициализации симуляции,
	•	взаимодействия с другими подсистемами модели.

Их можно настраивать и использовать на верхнем уровне в конфигураторе, CLI и блокнотах.

⚙️ Внутренние (служебные) классы (с подчёркиванием):

Их названия начинаются с _ и они предназначены для внутренних целей реализации:
	•	_ReaderOFF, _ReaderQUERY, _ReaderACK и т. д. — реализации состояний конечного автомата Reader.
	•	_PowerControlMode, _AlwaysPoweredOn, _PeriodicPowerOn — режимы питания.
	•	_ReaderSlot, _ReaderRound — внутреннее управление слотами и раундами.
	•	_TagPowerRecord, _TagReadRecord, _TagRecord — статистика на низком уровне.
	•	_TagPowerMinMap — вспомогательная структура для хранения мощности.
	•	_ReaderState, _ReaderAdjust и пр. — абстрактные или базовые классы состояний.

Это — вспомогательные строительные блоки. Их цель — реализовать сложную внутреннюю логику (например, протокол EPC Gen2) без необходимости раскрывать эти детали пользователю модели.
