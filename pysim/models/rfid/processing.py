...
def result_processing(initial_data, results, variadic, save_results=False):
    ...
    if isinstance(results, list):
        ...
        ret_cols = ["read_tid_prob", "inventory_prob", "rounds_per_tag", "read_tid_time"]
        results_table = [
            [round(getattr(results, key), 3) for key in ret_cols]
        ]
        print(tabulate(results_table, headers=ret_cols, tablefmt="pretty"))
        print(f"Время выполнения симуляции: {round(results.execution_time, 3)} с.")


def separate_res(results):
    ret = results
    time_list = [res.execution_time for res in results]
...
def results_to_dict(ret):
    ...
    for item in ret:
        results_dict["read_tid_prob"].append(getattr(item, "read_tid_prob"))
        results_dict["inventory_prob"].append(getattr(item, "inventory_prob"))
        results_dict["rounds_per_tag"].append(getattr(item, "rounds_per_tag"))
...
def print_the_mult_results_to_the_terminal(initial_data, results, variadic):
    ...
    params_names = [
        ...
        "tag_offset", "power", "encoding", "tari", "num_tags", "q"
    ]
    ...
    results_table = [
        [round(getattr(item, column), 3) for column in ret_cols]
        for item in ret
    ]
...
def plot_results(initial_data, results, variadic, save_fig = True):
    ...
    units = {"speed": "km/h", "reader_offset": "m", "altitude": "m",
             "power": "dBm", "tid_word_size": "words", "q": ""}
